...
// Menú con createPortal a document.body para no quedar recortado por overflow:hidden del padre
// Sin soporte de teclado (solo mouse) — por eso el escaneo de código de barras usa BarcodeScanInput aparte
export function Combobox({
  ...
  const displayedOptions = isControlled
    ? options
    : options.filter(
        (o) => !localSearch || o.label.toLowerCase().includes(localSearch.toLowerCase()),
      )
  ...
  return (
    <div ref={triggerRef} className="relative">
      ...
      >
        ...
        <ChevronDown
          className={cn(
            'w-4 h-4 text-content-faint shrink-0 transition-transform',
            open && 'rotate-180',
          )}
        />
      </div>
            ...
            <div className="max-h-52 overflow-y-auto">
              ...
              {!isLoading && displayedOptions.length === 0 && (
                <p className="text-xs text-content-faint text-center py-4">
                  {isControlled && searchValue.length === 0
                    ? 'Escribe para buscar...'
                    : 'Sin resultados'}
                </p>
              )}
