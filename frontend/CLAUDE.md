...
| `/products` | Done | Full CRUD, server-side search, debounce, pagination, cache, stock column (total + per-warehouse breakdown), columnas de reserva ("PREVENTA" = `reservedQuantity`, "EN REMISIÓN" = `remisionQuantity`, "Disponible" = `availableStock` ya neteado), cost columns ("Costo Prom." / "Últ. Costo") |
