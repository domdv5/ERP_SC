...
**Chip de conversión PV + gating por derivada activa** — El backend adjunta en cada documento un bloque computado `pv: { conversion: { status: 'none' | 'pending' | 'converted', documents: PvDerivedDocRef[] } } | null` (null salvo `type === 'PV'`; `documents` trae TODAS las derivadas POS/COT, incluidas las anuladas). En listado (celda Estado, bajo el pill) y detalle (junto a los pills del título) se muestra un chip `PV_CONVERSION_BADGE` — "En conversión" (ámbar) para `pending`, "Convertida" (violeta) para `converted`, nada para `none`. Un contador de antigüedad (`daysSince`/`formatDaysSince` en `lib/utils.ts`, calculado en el front desde `createdAt`) aparece solo mientras la PV sigue abierta (`confirmed` + `status === 'none'`). En el detalle, si existe una derivada no anulada (`documents.find(d => d.status !== 'voided')`) se **ocultan** los botones "Anular" y "Convertir a venta" y en su lugar se ofrece "Ver venta {ref}" que navega a esa venta; si todas las derivadas están anuladas, los botones vuelven a su gating normal. Esto es solo prevención en UI — el backend igual responde 409 al anular/reconvertir una PV con derivada activa.
