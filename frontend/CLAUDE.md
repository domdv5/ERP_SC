...
**Sidebar groups** — `Sidebar.tsx` tiene un grupo **"Ventas"** (paralelo a "Operaciones"/"Finanzas") para pantallas de venta. Ítems: **"Nueva venta"** → `/documents/pos/new` (checkout compartido contado/crédito, gate `canSell = canCreatePOS || canCreateCOT`) y **"Nueva remisión"** → `/documents/new?type=REM` (form genérico de documentos, gate `document.create.REM`, ícono `Truck`). El grupo entero se muestra con `showVentas = canSell || canCreateREM`. "Operaciones" es solo compras/ajustes/traslados de inventario, no mezclar. DVV debe nacer en "Ventas" cuando se implemente, no en "Operaciones".
