...
**Sidebar groups** — Los grupos y su orden ("Maestros", "Operaciones", "Ventas", "Finanzas", "Administración") están en `config/navigation.ts`. El grupo **"Ventas"** (paralelo a "Operaciones"/"Finanzas") es para pantallas de venta: **"Nueva venta"** → `/documents/pos/new` (checkout contado/crédito, permiso `['document.create.POS','document.create.COT']`) y **"Nueva remisión"** → `REM_NEW_PATH` = `/documents/new?type=REM` (form genérico, permiso `document.create.REM`, ícono `Truck`). "Operaciones" es solo compras/ajustes/traslados de inventario, no mezclar. DVV debe nacer en "Ventas" cuando se implemente, no en "Operaciones".
...
| `/documents` | Done | List + form (create/edit/confirm/void), portal Combobox, search-on-type. Incluye `/documents/pos/new` (checkout dedicado contado/crédito con toggle POS/COT) y conversión PV/REM→POS / PV/REM→COT desde el detalle. REM (remisión) usa el form genérico (`?type=REM`), comparte columnas de reserva y panel de liberación con PV. |
